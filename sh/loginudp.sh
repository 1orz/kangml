...
then
echo $(date +%Y年%m月%d日%k时%M分) "用户通过UDP登录成功" "账号:"${username}>>/home/wwwroot/default/res/login.log
exit 0
else
echo $(date +%Y年%m月%d日%k时%M分) "用户通过UDP登录失败" "账号:"${username}>>/home/wwwroot/default/res/login.log
exit 1
fi